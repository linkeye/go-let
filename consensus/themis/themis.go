...
func accumulateRewards(config *params.ThemisConfig, state *state.StateDB, header *types.Header) error {
	...
	// begin with 48 reward
	var (
		basicReward      = float64(48)
	...
	//state.AddBalance(header.Coinbase, reward)
	log.Debug("themis accumulateRewards","basicReward",basicReward,"yearNumber",yearNumber,"currentReward",currentReward)
